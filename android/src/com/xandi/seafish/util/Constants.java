package com.xandi.seafish.util;

public class Constants {

    public static final String DATABASE_REF_RANKING = "ranking";
    public static final String DATABASE_REF_USER = "user";
    public static final String DATABASE_REF_SCORE = "score";
    public static final String DATABASE_REF_PERSONAL_SCORE = "personalScore";
    public static final int RANKING_SIZE = 100;

    //Legal documents
    public static final String PRIVACY_POLICY = "file:///android_asset/privacypolicy.html";
    public static final String TERMS_AND_CONDITIONS = "file:///android_asset/termsconditions.html";
}